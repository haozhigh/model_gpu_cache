...
for suite in $suites; do
    ...
    for bench in $benches; do
		...
		if [ "$1" == "" -o \( "$1" != "" -a "$1" == "$bench" \) ]; then
        	echo "##  gen_code: Generate code for $suite/$bench  ##"
			cd "$build_dir"

			##  Make sure the dir to store trace exists
			out_dir_bench="$code_dir/$suite/$bench"
            ...
            log_dir_suite="$log_dir_code/$suite"
            makesure_dir_exists "$log_dir_suite"

			##  Generating code
            ##  Get the time stamp before and after execution
            stamp0=$( get_time_ms )
			./${bench}_code "$out_dir_bench" $( get_bench_args $suite $bench ) | tee "$$log_dir_suite/$bench.log"
            stamp1=$( get_time_ms )
