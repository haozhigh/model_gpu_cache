...
for suite in $suites; do
    ...
    for bench in $benches; do
		...
		if [ "$1" == "" -o \( "$1" != "" -a "$1" == "$bench" \) ]; then
        	echo "##  gen_thread_limit_trace.sh: Generate trace for $suite/$bench  ##"
			cd "$build_dir"

			##  Make sure the dir to store trace exists
			out_dir_bench="$thread_limit_trace_dir/$suite/$bench"
            ...
            log_dir_suite="$log_dir_thread_limit_trace/$suite"
            makesure_dir_exists "$log_dir_suite"

			##  Generating trace
            ##  Get the time stamp before and after execution
            stamp0=$( get_time_ms )
			./${bench}_trace "$out_dir_bench" 0 $( get_bench_args $suite $bench ) | tee "$log_dir_suite/$bench.log"
            stamp1=$( get_time_ms )
