...
for suite in $suites; do
    ...
    for bench in $benches; do
		...
		if [ "$1" == "" -o \( "$1" != "" -a "$1" == "$bench" \) ]; then
            ...
            log_dir_maxwell_model_suite="$log_dir_maxwell_model/$suite"
            makesure_dir_exists "$log_dir_maxwell_model_suite"

            ##  Make log_file empty
            ##  One log file to store all kernel modeling log
            echo "" > "$log_dir_maxwell_model_suite/$bench.log"
            ...
            for kernel_name in $kernel_names; do
                ...
                ./model "$trace_dir_bench/${kernel_name}.trc" "$trace_dir_bench/${kernel_name}.maxwell.distance" "$model_config_file" | tee -a "$log_dir_maxwell_model_suite/$bench.log"
            done

            ##  Calculate time duration and write to corresponding file
            stamp1=$( get_time_ms )
            duration=$((stamp1 - stamp0))
            echo $duration > "$log_dir_maxwell_model_suite/$bench.duration"
		fi
    done
done
