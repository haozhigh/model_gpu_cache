...
model_config_file="$build_dir/model_config/opt_break_jam_off_fermi_16k.config"
...
for suite in $suites; do
    ...
    for bench in $benches; do
		...
		if [ "$1" == "" -o \( "$1" != "" -a "$1" == "$bench" \) ]; then
        	echo "##  opt_break_jam_off: Running model for $suite/$bench  ##"
            ...
            out_dir_bench="$out_opt_break_dir/stack_trace_on/$suite/$bench"
            makesure_dir_exists "$out_dir_bench"

            ##  Make sure the dir to store log for this suite exists
            log_dir_suite="$log_dir_opt_break/stack_trace_on/$suite"
