...
for suite in $suites; do
    ...
    for bench in $benches; do
		...
		if [ "$1" == "" -o \( "$1" != "" -a "$1" == "$bench" \) ]; then
        	echo "##  opt_break_trace_off_run.sh: Running model for $suite/$bench  ##"
            ...
            out_dir_bench="$out_opt_break_dir/trace_off/$suite/$bench"
            makesure_dir_exists "$out_dir_bench"

            ##  Make sure the dir to store log for this suite exists
            log_dir_suite="$log_dir_opt_break/trace_off/$suite"
